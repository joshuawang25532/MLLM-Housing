...
def main():
    ...
    parser.add_argument(
        ...
        help="Source directory with raw scraped houses (default: data/raw_houses)"
    )
    parser.add_argument(
        "--k",
        type=int,
    ...
    if not args.skip_validation:
        ...
        run_command(
            ["python3", "-m", "utils.data_validator", "--dir", "data/raw_houses", "--output-issues", "data/issues.json"],
            "House Validation"
        )
    else:
    ...
    run_command(
        ["python3", "-m", "scripts.pipeline.preprocess_data", "--source", "data/raw_houses", "--dest", "data/preprocessed_houses"],
    ...
    run_command(
        ["python3", "-m", "scripts.pipeline.encode_features", "--source", "data/preprocessed_houses", "--dest", "data/encoded_houses"],
    ...
    if not args.skip_imputation:
        print("\n🤖 Stage 4: KNN imputation...")
        run_command(
            ["python3", "-m", "scripts.pipeline.impute_missing", "--source", "data/encoded_houses", "--dest", "data/imputed_houses", "--k", str(args.k)],
        ...
        run_command(
            ["python3", "-m", "scripts.pipeline.json_to_csv", "--source", "data/imputed_houses", "--output", "data/final_dataset.csv"],
        ...
        run_command(
            ["python3", "-m", "scripts.pipeline.json_to_csv", "--source", "data/encoded_houses", "--output", "data/final_dataset.csv"],
            "Single File Conversion"
        )
    ...
    print("\n📁 Output directories:")
    print(f"   - data/preprocessed_houses/        (preprocessed)")
    print(f"   - data/encoded_houses/  (one-hot encoded)")
    if not args.skip_imputation:
        print(f"   - data/imputed_houses/         (imputed)")
        print(f"   - data/final_dataset.csv      (single file)")
    print("=" * 70)
